...
def featuresToBoxes(outputs, anchors, n_classes, net_input_shape, threshold):
    grid_shape = outputs.shape[1:3]
    n_anchors = len(anchors)

    # Numpy fix to get the boxes in reasonable amount of time
    ...
    # net_input_shape is supposed squared and same as tensor_size
    tl_x = np.multiply(bx - half_bw, net_input_shape[1])
    tl_y = np.multiply(by - half_bh, net_input_shape[1]) 
    br_x = np.multiply(bx + half_bw, net_input_shape[1])
    br_y = np.multiply(by + half_bh, net_input_shape[1])
